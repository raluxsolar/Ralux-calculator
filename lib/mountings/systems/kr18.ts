...
// Base starting attachments (4/6/8 logic is the same as Micro/Mini)
const BASE_ATTACHMENTS_PER_MODULE = 4;
// ✅ KR18: screws per attachment fixed at 1
const SCREWS_PER_ATTACHMENT = 1;
...
function getWindPressurePsf(sr: SurroundingsResult, pos: Pos): number {
  ...
  throw new Error("KR18 currently requires USA – ASCE 7 surroundings result.");
}
...
// percent rule: round UP (ceil) to integer percent
...
export function calcKr18(input: MountingInput): MountingResult {
  ...
  // ✅ Fixed fail-mode values (no table / no thickness mapping)
  const shearFailMode = 300;
  const upliftTensionFailMode = 344;
  ...
  // for the report we just echo sheet thickness as the "used" value
  const tKeyUsed_mm = sheetThickness_mm;
  ...
  return {
    ...
    explain: {
      ...
      // now just equal to actual sheet thickness, no table snap
      tKeyUsed_mm,
    },

    meta: { packVersion: "2026.01.KR18.v2" },
  };
}
