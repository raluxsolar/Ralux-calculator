...
function getWindPressurePsf(sr: SurroundingsResult, pos: Pos): number {
  ...
  throw new Error(
    "Mini-rail currently requires USA – ASCE 7 surroundings result."
  );
}
...
const THICKNESS_KEYS = [
   0.45, 0.5, 0.55, 0.63, 0.75, 0.88, 1.0, 1.25,
] as const;
...
export function calcMiniRail(input: MountingInput): MountingResult {
  ...
  return {
    ...
    meta: { packVersion: "2026.01.MINI-RAIL.v1" },
  };
}
