// lib/mountings/systems/clip-kr18.ts
...
function getWindPressurePsf(sr: SurroundingsResult, pos: Pos): number {
  ...
  throw new Error("Clip-KR18 currently requires USA – ASCE 7 surroundings result.");
}
...
export function calcClipKr18(input: MountingInput): MountingResult {
  // ---------- Inputs ----------
  ...
  // ✅ Clip-KR18 caps ONLY difference
  const upliftCap = 346;
  const downforceCap = 346;
  const shearCap = 150;
  ...
  return {
    ...
    meta: { packVersion: "2026.01.CLIP-KR18.v1" },
  };
}
